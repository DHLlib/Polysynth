...
| **实时流式交互** | WebSocket 实时传输 LLM 流式 token，浏览器内逐字显示各角色发言 |
| **参与者配置** | 每个角色独立配置模型、名称、颜色、system prompt；颜色 Hex ↔ ANSI 双向转换 |
| **供应商管理** | 多 LLM 供应商动态配置（API Key、Base URL），模型与供应商绑定，运行时自动路由 |
| **文件上传** | 支持 txt/md/pdf/docx/xlsx/pptx，AI 提取摘要并注入所有角色 system prompt |
| **Agent 工具** | DuckDuckGo 搜索工具，双阶段调用，搜索结果写入历史供后续角色参考 |
| **轮次控制** | 元数据驱动：`configurable` 标志控制是否可前台调整（六顶思考帽 1~10 轮，辩论赛固定 4 轮） |
| **会话历史** | 侧边栏展示所有历史会话，点击回溯完整记录，切换会话时历史与实时事件合并渲染 |
| **双模式运行** | Web 模式（React + FastAPI + SQLite）与 CLI 模式共享同一套核心引擎 |
| **数据持久化** | SQLite 异步存储 + JSON 配置文件作为默认 seed + `.jsonl` 运行时备份 |
...
部分角色可启用工具调用（通过 `tools_enabled` JSON 数组配置，如 `["search"]`）：

- **DuckDuckGo 搜索**：免费搜索 API，支持 `backend="html"` 和 `"lite"` 双回退（适配中国大陆网络）
- **双阶段工具调用**：Phase 1 非流式检测 `tool_calls` → 执行搜索 → Phase 2 流式输出基于搜索结果的最终答案
- **搜索关键词优化**：调用搜索前，使用当前角色模型根据 topic + history 生成精准搜索词
- **工具结果写入历史**：确保后续角色能看到之前的搜索记录，避免重复搜索
...
- **文件上传**（拖放/点击）：支持 txt/md/pdf/docx/xlsx/pptx，单文件 20MB，最多 5 个文件
  - 文件内容提取为文本 → AI 生成结构化摘要 → 注入所有角色的 system prompt 作为背景资料
...
pip install PyMuPDF python-docx openpyxl python-pptx  # 文件解析（可选）
...
|------|--------|------|
| ~~Web UI~~ | ~~高~~ | ✅ 已完成 |
| ~~文件上传~~ | ~~高~~ | ✅ 已完成（txt/md/pdf/docx/xlsx/pptx + AI 摘要注入） |
| ~~Agent 工具（搜索）~~ | ~~高~~ | ✅ 已完成（DuckDuckGo 双回退搜索） |
